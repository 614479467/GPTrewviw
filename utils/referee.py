...
class GPTRefereeBase:
    ...
    def __init__(self, 
                 ...
                 output_finegrained_analysis: bool=True,
                 n_processes: int=50):
        ...
        self.output_finegrained_analysis = output_finegrained_analysis
        ...
        self.task_dir = os.path.join(self.pair_dir, evaluation_method)
        self.setting_dir = os.path.join(self.task_dir, setting)
    ...
    def _init_referee(self, referee: str):
        ...
        if referee == 'gpt-3.5-turbo':
            from gpt import GPT
            self.referee = GPT(user_name='evaluation')
    ...
    def _read_sample(self, model_name) -> None:
        ...
        with open(os.path.join(self.data_dir, f"{model_name}.jsonl"), encoding='utf-8', mode='r') as reader:
            return [json.loads(line) for line in reader.read().strip(' \n').split('\n')]
    ...
    def _read_samples(self) -> None:
        ...
        """
        self.sample_list =  {
            self.model_a_name: sorted(self._read_sample(self.load_model_a_name), key=lambda x: x['id']),
            self.model_b_name: sorted(self._read_sample(self.load_model_b_name), key=lambda x: x['id']),
        }
        assert [a['id'] == b['id'] for a, b in zip(self.sample_list[self.model_a_name], self.sample_list[self.model_b_name])], "all ids should be matching!"
    ...
    def _request_eval_gpt4_api(self, content):
        ...
        eval_result = self.referee(content, args=self.args)
        if eval_result == "":
            raise ValueError
        return eval_result
    ...
    def request_eval(self):
        ...
        for times_idx in range(self.n_repeat):
            for sample in tqdm(self.eval_list[times_idx], desc="Processing samples", unit="sample"):
    ...
    def request_eval_mp(self):
        ...
        with multiprocessing.Pool(processes=self.n_processes) as pool:
            results = [
                ...
                for times_idx in range(self.n_repeat) for sample in self.eval_list[times_idx]
                if not os.path.exists(os.path.join(self.files_dir, str(times_idx), str(sample["id"]) + ".json"))
            ]
    ...
    def _decode_scores(self, samples: list) -> List[Tuple[float, float]]:
        ...
        for sample in samples:
            ...
            if sample["assistant1_name"].lower() == self.model_b_name.lower():
    ...
    def _compute_metrics(self, scores_list: List[Tuple[float, float]]):
        ...
        assistant1_metrics.update({
            'Performance Ratio': self._save_divide(assistant1_metrics['Avg Score'], assistant2_metrics['Avg Score'])
        })
        assistant2_metrics.update({
            'Performance Ratio': self._save_divide(assistant2_metrics['Avg Score'], assistant1_metrics['Avg Score'])
        })
    ...
    def _compute_metrics_for_aggregated_samples(self, scores_list: List[Tuple[list, list]]):
        ...
        assistant1_metrics.update({
            f'Performance Ratio {self.n_repeat}': self._save_divide(assistant1_metrics[f'Avg Score {self.n_repeat}'], assistant2_metrics[f'Avg Score {self.n_repeat}'])
        })
        assistant2_metrics.update({
            f'Performance Ratio {self.n_repeat}': self._save_divide(assistant2_metrics[f'Avg Score {self.n_repeat}'], assistant1_metrics[f'Avg Score {self.n_repeat}'])
        })
        return assistant1_metrics, assistant2_metrics
    ...
    def _aggregate_metrics(self, metrics_list: List[Tuple[dict, dict]]):
        ...
        ratio_list1 = [self._save_divide(metrics1['Avg Score'], metrics2['Avg Score']) for metrics1, metrics2 in zip(metrics1_list, metrics2_list)]
        ratio_list2 = [self._save_divide(metrics2['Avg Score'], metrics1['Avg Score']) for metrics1, metrics2 in zip(metrics1_list, metrics2_list)]
        assistant1_metrics.update({
            'Performance Ratio': {
                f'All {self.n_repeat}': ratio_list1,
                f'AVG {self.n_repeat}': self._get_avg_safe(ratio_list1),
                f'STD {self.n_repeat}': self._get_std_safe(ratio_list1),
            }
        })
        assistant2_metrics.update({
            'Performance Ratio': {
                f'All {self.n_repeat}': ratio_list2,
                f'AVG {self.n_repeat}': self._get_avg_safe(ratio_list2),
                f'STD {self.n_repeat}': self._get_std_safe(ratio_list2),
            }
        })
        return assistant1_metrics, assistant2_metrics
    ...
    @staticmethod
    def plot_radar_map(all_metrics: Dict[str, float], model_name: str, base_name: str, output_dir: str):
        """Plot a rader map"""

        file_name = f'radar_{model_name}.png'
        ...
        if any(contains_chinese(c) for c in categories):
            plt.rcParams['font.sans-serif'] = ['SimSun']

        fig, ax = plt.subplots(figsize=(8, 8), subplot_kw=dict(polar=True))
        values = [all_metrics[c] for c in categories]
        values_closed = values + values[:1]
        ax.plot(theta, values_closed, 'o-', linewidth=2, label=model_name)
        ax.fill(theta, [1] * len(theta), alpha=0.25, label=base_name)
        ax.grid(linewidth=1)

        ax.set_xticks(theta[:-1])
        ax.set_xticklabels(categories)
        ax.get_xticklabels()[0].set_color('red'); ax.get_xticklabels()[0].set_fontweight('bold')
        ax.legend(loc=(0.86,0.85))

        ax.set_title(f'Performance ratio of {model_name} when setting {base_name} as baseline')
        plt.savefig(os.path.join(output_dir, file_name), bbox_inches='tight', dpi=600)

    @staticmethod
    def plot_histogram(metrics_a: Dict[str, float], metrics_b: str, model_a_name: str, model_b_name: str, output_dir: str):
        """Plot a histogram"""
        file_name = f'histogram.png'

        categories = sorted(metrics_a.keys())
        categories.remove('Total')
        categories = ['Total'] + categories
        win_a = [metrics_a[c] for c in categories]
        win_b = [metrics_b[c] for c in categories]
        tie = [1 - a - b for a, b in zip(win_a, win_b)]
        ...
        width = 0.25
        plt.bar(x - width, win_a, width, label=model_a_name)
        plt.bar(x, tie, width, label='tie')
        plt.bar(x + width, win_b, width, label=model_b_name)

        _, xticks = plt.xticks(x, categories, rotation=45)
        xticks[0].set_color('red'); xticks[0].set_fontweight('bold')
        plt.ylim(0, 1.2)
        plt.ylabel('Win Rate')
        plt.legend()

        plt.title(f'Win rate of {model_a_name} v.s. {model_b_name}')
        plt.savefig(os.path.join(output_dir, file_name), bbox_inches='tight', dpi=600)


    def plot_fine_grained_figure(self):
        """Use `metrics.json` to plot performance ratio / win rate over different categories"""
        metrics_file = os.path.join(self.setting_dir, 'metrics.json')
        all_metrics = json.load(open(metrics_file, encoding='utf-8'))

        categories = sorted(all_metrics.keys())
        if self.evaluation_method.startswith('scoring'):
            target_metric = 'Performance Ratio'
        elif self.evaluation_method.startswith('classification'):
            target_metric = 'Win Rate'

        metrics_a = {c: all_metrics[c][self.model_a_name][target_metric][f'AVG {self.n_repeat}'] for c in categories}
        metrics_b = {c: all_metrics[c][self.model_b_name][target_metric][f'AVG {self.n_repeat}'] for c in categories}

        if self.evaluation_method.startswith('scoring'):
            self.plot_radar_map(metrics_a, model_name=self.model_a_name, base_name=self.model_b_name, output_dir=self.setting_dir)
            self.plot_radar_map(metrics_b, model_name=self.model_b_name, base_name=self.model_a_name, output_dir=self.setting_dir)
        elif self.evaluation_method.startswith('classification'):
            self.plot_histogram(metrics_a, metrics_b, model_a_name=self.model_a_name, model_b_name=self.model_b_name, output_dir=self.setting_dir)
    ...
    def review(self):
        ...
        if self.output_finegrained_analysis:
            self.plot_fine_grained_figure()
...
class GPTReferee_NonChat(GPTRefereeBase):
    ...
    def __init__(self, 
        ...
        super(GPTReferee_NonChat, self).__init__(model_a_name=model_a_name,
                                                 ...
                                                 n_repeat=n_repeat,
                                                 setting=setting,
                                                 output_finegrained_analysis=output_finegrained_analysis,
